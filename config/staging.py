...
if not load_dotenv(join(BASE_DIR, "config/staging.env")):
    raise Exception(
        "Failed to load environment variables from staging.env file"
    )

if APP_USES_DATABASE:
    DB_HOSTNAME = os.environ.get("DB_HOSTNAME", "serverdb")
    DB_PORT = os.environ.get("DB_PORT", "5432")
    DB_NAME = os.environ.get("DB_NAME", "staging_db")
...
print("--=== Staging environment configuration loaded successfully. ===--")
